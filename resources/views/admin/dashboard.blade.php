...
<x-ui.card padding="p-0" class="mb-8 overflow-hidden">
  <div class="border-b border-slate-100 px-5 py-4 sm:px-6">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
      <div>
        <h2 class="text-base font-bold text-slate-900">Registered organizations</h2>
...
@if (auth()->user()?->isSuperAdmin())
  <x-ui.card padding="p-5" class="mb-8">
...
<section class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
  @foreach ($overview as $item)
    <a href="{{ $item['route'] }}" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-300/40 transition duration-200 hover:-translate-y-0.5 hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-[#003E9F]/15">
      ...
      <span class="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-[#003E9F]">
        Open module
...
<section class="mt-8">
  <div class="mb-4">
    <h2 class="text-base font-bold text-slate-900">Centralized Activity Calendar</h2>
