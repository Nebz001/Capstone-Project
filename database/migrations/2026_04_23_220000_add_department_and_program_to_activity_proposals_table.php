...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (Schema::hasTable('organizations')) {
            // Backfill legacy records safely across DB variants.
            if (Schema::hasColumn('organizations', 'college_department')) {
                DB::statement("
                    ...
                    SET ap.program = COALESCE(NULLIF(ap.program, ''), NULLIF(o.college_department, ''))
                    WHERE ap.program IS NULL OR ap.program = ''
                ");
            }
            ...
            if ($schoolSourceColumn !== null) {
                DB::statement("
                    UPDATE activity_proposals ap
                    INNER JOIN organizations o ON o.id = ap.organization_id
                    SET ap.school_code = COALESCE(NULLIF(ap.school_code, ''), NULLIF(o.{$schoolSourceColumn}, ''))
                    WHERE ap.school_code IS NULL OR ap.school_code = ''
                ");
            }
        }
    }
